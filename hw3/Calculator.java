...
public class Calculator extends JFrame {
   ...
   public Calculator() {
      setSize(new Dimension(5*(buttonWidth+10)-30, 400));
      setTitle("Calculator");
   ...
   private void processEntry(){
      ...
      if(infixString.length() == 0)
        entryField.setText("Empty");
      
      else if(infixString.length() - infixString.replace("(", "").length() != infixString.length() - infixString.replace(")", "").length()){
         entryField.setText("Mismatched Parenthese");
      }
      
      else if(infixString.substring(0,1).matches("[0-9]")||infixString.substring(0,1).matches("[(]")){
      ...
      try{
        ...
        entryField.setText("Other Exception");
      }
   }
   else
     entryField.setText("Wrong format");
   }
